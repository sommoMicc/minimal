...
# folder will be "minimal/src/work/overlay/mll_migration".
mkdir $OVERLAY_WORK_DIR/$BUNDLE_NAME
cp $OVERLAY_SOURCE_DIR/migration.c \
...
# folder is "minimal/src/work/overlay/mll_migration/mll_migration_installed".

# First we create the "destination" folder.
mkdir $DEST_DIR

# We want our "migration" executable to reside in the "/bin" folder, so we create
# subfolder "bin" in "$DEST_DIR".
mkdir $DEST_DIR/bin

# We want our "autorun" script file "90_migration.sh" to reside in "/etc/autorun",
# so we create the corresponding folders in "$DEST_DIR".
mkdir -p $DEST_DIR/etc/autorun

# Now we copy the "autorun" script file "90_migration.sh" in "$DEST_DIR/etc/autorun"
# and we make sure the script is executable.
cp $SRC_DIR/90_migration.sh $DEST_DIR/etc/autorun
chmod +x $DEST_DIR/etc/autorun/90_migration.sh

# Now we can compile "$OVERLAY_WORK_DIR/$BUNDLE_NAME/migration.c" and place it in
# "$DEST_DIR/bin" as executable binary "migration".
gcc -o $DEST_DIR/bin/migration $OVERLAY_WORK_DIR/$BUNDLE_NAME/migration.c -lm
...
reduce_size $DEST_DIR/bin/migration
